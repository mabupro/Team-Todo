...
const GROUP_RANGES: Record<Group, DateRange> = {
...
export function useCalendarLogic() {
  ...
  const onChangeGroup = useCallback((g: string) => {
    setSelectedGroup(g as Group);
  }, []);
  ...
  const jpHolidays = useMemo(() => {
    ...
    return hd.getHolidays(today.getFullYear()).map((h) => {
